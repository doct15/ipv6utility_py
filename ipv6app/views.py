...
def custom_proc(request):
    ...
    ztemp2=""
    flag = hexstring.find(ipv6address_truncate[17:18])
    if (flag & 2):
        isglobal = "global"
    else:
        isglobal = "local"
...
def convertinput(inipv6address):
    #clearall()
    global htmltext
    global ipv6address
    global ipv6address_truncate
    ...
    while (currentchar < len(convertstring)):
        ...
        print currentchar,zchar
        if zchar == ".":
            if chunk[currentchunk] == "":
                alert('Bad ipv4 address embedding!')
                return False
            if currentoctet>0:
            ...
            if currentchar+1 <= len(convertstring):
                for ztemp in range (currentchar+1, len(convertstring)):
                    ...
                    if zchar2 == ".":
                        ...
                        if currentoctet<3:
                            alert('Not enough octets in ipv4 address!')
                            return False
                        break
                    elif not re.match(r'[0-9]',zchar2):
                        alert('Invalid char in ipv4 address!')
                        return False
                    else:
        ...
        if zchar == "/":
            ...
            if (subnet>128) or (subnet<0):
                alert('Bad prefix bits!')
                return False
        elif zchar == ":":
            if lastchar == ":":
                if doublecolon > -1:
                    alert('More than 1 double colon!')
                    return False
            ...
            if len(chunk[currentchunk])>3:
                alert('Too many in chunk!')
                return False
            zchar=zchar.upper()
            chunk[currentchunk] = chunk[currentchunk]+zchar
        else:
            alert('Illegal character in address['+zchar+']!')
            return False
        lastchar=zchar
        currentchar += 1
    while (len(chunk[currentchunk])<4):
    ...
    if maxchunks<7:
        if doublecolon == -1:
            alert('Bad chunk count! Missing doublecolon?')
            return False
...
def checkmasks():
    ...
    for detail in details:
        ...
        if not zchar:
            alert ('Bad details_ip Conversion.')
            return False
        zprefix=ipv6address
        ...
        if details_mask != "":
            ...
            if not zchar:
                alert ('Bad details_mask Conversion.')
                return False
        ...
        if zchar != "No":
            ...
            if details_flag == "6to4":
                #6to4;
                #v4addr=getv4(substr(oipv6address_truncate,4,8))
                v4addr=getv4(oipv6address_truncate[4:12])
                print oipv6address_truncate[4:12],v4addr
                ...
                #ISATAP;
                #v4addr=getv4(substr($oipv6address_truncate,24,8))
                ...
                flag = hexstring.find(oipv6address_truncate[17:18])
                flag2 = hexstring[flag^2:flag^2+1]
                ...
                #print "$cc.startrow1...MAC 48$cc.rowdivider2 Possible$cc.rowdivider2$MAC48$cc.endrow1\n";
                ...
                #print "$cc.startrow1...24bits$cc.rowdivider1$bit24$cc.endrow1\n";
                ...
                if istrue:
                    ...
                    ispriv="Public"
                if v4addr == "0.0.0.0":
                    #print "$cc.startrow2...v4 compatible v4$cc.rowdivider2$v4addr - NOT v4-compatible. $cc.endrow1\n";
                    htmltext.append(cc.startrow2+"...v4 compatible v4"+cc.rowdivider2+v4addr+" - NOT v4-compatible."+cc.endrow1)
                else:
                    #print "$cc.startrow2...v4 compatible v4$cc.rowdivider2$v4addr - $ispriv ipv4 address. $cc.endrow1\n";
                ...
                if istrue:
                    ...
                    ispriv="Public"
                #print "$cc.startrow2...v4 mapped v4$cc.rowdivider2$v4addr - $ispriv ipv4 address. $cc.endrow1\n";
                ...
                #print "$cc.startrow1$cc.rowdivider1...Above is anyscope multicast ff0X::\n";
...
def binToHex(bin):
    ...
    if (((float(ztemp3) / 4) != int(ztemp3 / 4)) or (ztemp3 < 4) or (ztemp3 > 128)):
        alert('Wrong bit length for hex conversion!')
        return False
    ztemp3=""
...
def zand(and1, and2):
    if ((len(and1) != len(and2)) or (len(and1) != 128)):
        alert("Can't AND non 128bit lengths!")
        return False
    ztemp3=""
...
def zchunk(address):
    #my $address=shift(@_);
    if (len(address) != 32):
        alert("Can't chunk non 32 hex string!")
        return False
    ztemp2=""
...
def unchunk(address):
    #my $address=shift(@_);
    if (len(address) != 39):
        alert("Can't unchunk non 39 hex string!")
        return False
    ztemp2=""
...
def getv4(hex):
    #my $hex=shift(@_);
    if (len(hex) != 8):
        alert("Can't get IPv4 address from non 8 hex string!")
        return False
    zchar=""
    ztemp=0
    v4addr=""
...
def hexToOctet(hex):
    ...
    if (len(hex) != 2):
        ...
        return false
    #high = index($hexstring,substr($hex,0,1)) * 16
    #low = index($hexstring,substr($hex,1,1))
